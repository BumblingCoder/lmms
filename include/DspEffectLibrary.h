...
public:
	MonoToStereoAdaptor( const FXL & monoFX )
	    : m_leftFX( monoFX ), m_rightFX( monoFX )
	{
	}

	MonoToStereoAdaptor( const FXL & leftFX, const FXR & rightFX )
	    : m_leftFX( leftFX ), m_rightFX( rightFX )
	{
	}
...
{
public:
	StereoToMonoAdaptor( const FX & fx ) : m_FX( fx ) {}
...
{
public:
	FastBassBoost( const sample_t _frequency, const sample_t _gain,
...
public:
	FoldbackDistortion( float threshold, float gain )
	    : m_threshold( threshold ), m_gain( gain )
	{
	}
	...
	sample_t nextSample( sample_t in )
	{
		...
		{
			return ( fabsf(
			             fabsf( fmodf( in - m_threshold, m_threshold * 4 ) ) -
			             m_threshold * 2 ) -
			         m_threshold ) *
			       m_gain;
		}
...
public:
	...
	sample_t nextSample( sample_t in )
	{
		return m_gain * ( in * ( fabsf( in ) + m_threshold ) /
		                  ( in * in + ( m_threshold - 1 ) * fabsf( in ) + 1 ) );
	}
...
{
public:
	StereoEnhancer( float wideCoeff ) : m_wideCoeff( wideCoeff ) {}
