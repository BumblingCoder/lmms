...
public:
	...
	inline void setCoeffs( float freq )
	{
		...
		m_a = 1.0 /
		      ( 4.0 * wc2 * k2 + 2.0 * sq_tmp1 + m_k4 + 2.0 * sq_tmp2 + m_wc4 );
		...
		m_b4 =
		    ( m_k4 - 2.0 * sq_tmp1 + m_wc4 - 2.0 * sq_tmp2 + 4.0 * wc2 * k2 ) *
		    m_a;
	}
	...
	inline float update( float in, ch_cnt_t ch )
	{
		...
		const double y = ( m_a0 * x ) + ( m_z1[ch] * m_a1 ) +
		                 ( m_z2[ch] * m_a2 ) + ( m_z3[ch] * m_a1 ) +
		                 ( m_z4[ch] * m_a0 );
		m_z4[ch] = m_z3[ch];
		m_z3[ch] = m_z2[ch];
		m_z2[ch] = m_z1[ch];
...
public:
	...
	inline sample_t update( sample_t _in0, ch_cnt_t _chnl )
	{
		...
		switch( m_type )
		{
			case Moog:
			{
				...
				// (bilinear transform)
				m_y1[_chnl] = qBound(
				    -10.0f, ( x + m_oldx[_chnl] ) * m_p - m_k * m_y1[_chnl],
				    10.0f );
			...
			case Tripole:
			{
				...
				for( int i = 0; i < 4; ++i )
				{
					...
					out += ( m_y3[_chnl] - m_y3[_chnl] * m_y3[_chnl] *
					                           m_y3[_chnl] * ( 1.0f / 6.0f ) );
				}
				out *= 0.25f;
				m_last[_chnl] = _in0;
				return out;
			...
			case Bandpass_SV:
			{
				...
				for( int i = 0; i < 2; ++i ) // 2x oversample
				{
					m_delay2[_chnl] =
					    m_delay2[_chnl] +
					    m_svf1 *
					        m_delay1[_chnl]; /* delay2/4 = lowpass output */
					highpass = _in0 - m_delay2[_chnl] - m_svq * m_delay1[_chnl];
					m_delay1[_chnl] =
					    m_svf1 * highpass +
					    m_delay1[_chnl]; /* delay1/3 = bandpass output */
			...
			case Notch_SV:
			{
				...
				// RC-Bandpass,-Lowpass,-Highpass- Filter-Network as it was used
				// in nearly all modern analog synthesizers. This can be driven
				// up to self-oscillation (BTW: do not remove the limits!!!).
				// (C) 1998 ... 2009 S.Fendt. Released under the GPL v2.0  or
				// any later version.
			...
			case Bandpass_RC24:
			{
				...
				for( int n = 4; n != 0; --n )
				{
					...
					// second stage gets the output of the first stage as
					// input...
					in = m_type == Highpass_RC24 ? hp + m_rcbp1[_chnl] * m_rcq
			...
			case FastFormant:
			{
				if( qAbs( _in0 ) < 1.0e-10f &&
				    qAbs( m_vflast[0][_chnl] ) < 1.0e-10f )
				{
					return 0.0f;
				...
				out = 0;
				const int os = m_type == FastFormant
				                   ? 1
	...
	inline void calcFilterCoeffs( float _freq, float _q )
	{
		...
		{
			_freq =
			    qBound( minFreq(), _freq, 20000.0f ); // limit freq and q for
			                                          // not getting bad noise
			                                          // out of the filter...
			...
			const float f0 =
			    1.0f / ( linearInterpolate( _f[vowel + 0][0], _f[vowel + 1][0],
			                                fract ) *
			             F_2PI );
			const float f1 =
			    1.0f / ( linearInterpolate( _f[vowel + 0][1], _f[vowel + 1][1],
			                                fract ) *
			             F_2PI );

			// samplerate coeff: depends on oversampling
			const float sr =
			    m_type == FastFormant ? m_sampleRatio : m_sampleRatio * 0.25f;
		...
		if( m_type == Moog || m_type == DoubleMoog )
		{
			// [ 0 - 0.5 ]
			const float f =
			    qBound( minFreq(), _freq, 20000.0f ) * m_sampleRatio;
			// (Empirical tunning)
			m_p = ( 3.6f - 3.2f * f ) * f;
			m_k = 2.0f * m_p - 1;
		...
		{
			const float f =
			    qBound( 20.0f, _freq, 20000.0f ) * m_sampleRatio * 0.25f;
		...
		{
			const float f =
			    sinf( qMax( minFreq(), _freq ) * m_sampleRatio * F_PI );
