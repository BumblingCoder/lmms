...
public:
	...
	AutomatableModel( DataType type, const float val = 0, const float min = 0,
	                  const float max = 0, const float step = 0,
	                  Model * parent = NULL,
	...
	static float copiedValue() { return s_copiedValue; }
	...
	template <class T> static T castValue( const float v )
	{
		return ( T )( v );
	}

	template <bool> static bool castValue( const float v )
	{
		return ( qRound( v ) != 0 );
	}
	...
	//! @return pointer to model's valueBuffer when s.ex.data exists, NULL
	//! otherwise
	ValueBuffer * valueBuffer();

	template <class T> T initValue() const
	{
	 ...
	 * connections) of AutomatableModel into specified DOM element using <name>
	 * as attribute/node name
	 * @param doc TODO
	...
	/*! \brief Loads settings (value, automation links and controller
	   connections) of AutomatableModel from specified DOM element using <name>
	   as attribute/node name */
	virtual void loadSettings( const QDomElement & element,
	                           const QString & name );

	virtual QString nodeName() const { return "automatablemodel"; }

	QString displayValue( const float val ) const;

	bool hasLinkedModels() const { return m_hasLinkedModels; }

	// a way to track changed values in the model and avoid using signals/slots
	// - useful for speed-critical code. note that this method should only be
	// called once per period since it resets the state of the variable - so if
	// your model has to be accessed by more than one object, then this function
	// shouldn't be used.
	bool isValueChanged()
	{
	...
	bool hasStrictStepSize() const { return m_hasStrictStepSize; }
...
private:
	...
	// prevent several threads from attempting to write the same vb at the same
	// time
...
#define defaultTypedMethods( type )                                        \
	...
	{                                                                      \
		return AutomatableModel::value<type>( frameOffset );               \
                                                                           ...
                                                                           \
	type initValue() const { return AutomatableModel::initValue<type>(); } \
                                                                           \
	type minValue() const { return AutomatableModel::minValue<type>(); }   \
                                                                           \
	type maxValue() const { return AutomatableModel::maxValue<type>(); }
...
public:
	FloatModel( float val = 0, float min = 0, float max = 0, float step = 0,
	            Model * parent = NULL, const QString & displayName = QString(),
	            bool defaultConstructed = false )
	    : AutomatableModel( Float, val, min, max, step, parent, displayName,
	                        defaultConstructed )
	{
	}
...
{
public:
	IntModel( int val = 0, int min = 0, int max = 0, Model * parent = NULL,
	          const QString & displayName = QString(),
	          bool defaultConstructed = false )
	    : AutomatableModel( Integer, val, min, max, 1, parent, displayName,
	                        defaultConstructed )
	{
	}
...
{
public:
	BoolModel( const bool val = false, Model * parent = NULL,
	           const QString & displayName = QString(),
	           bool defaultConstructed = false )
	    : AutomatableModel( Bool, val, false, true, 1, parent, displayName,
	                        defaultConstructed )
	{
	}
