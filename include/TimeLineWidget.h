...
public:
	...
	Q_PROPERTY(
	    QColor barNumberColor READ getBarNumberColor WRITE setBarNumberColor )
	...
	Q_PROPERTY(
	    int loopRectangleVerticalPadding READ getLoopRectangleVerticalPadding
	        WRITE setLoopRectangleVerticalPadding )
	...
	inline void
	setLoopRectangleVerticalPadding( int const & loopRectangleVerticalPadding )
	{
	...
	inline void savePos( const MidiTime & _pos ) { m_savedPos = _pos; }
	inline const MidiTime & savedPos() const { return m_savedPos; }
	...
	inline virtual QString nodeName() const { return "timeline"; }

	inline int markerX( const MidiTime & _t ) const
	{
		return m_xOffset + static_cast<int>( ( _t - m_begin ) * m_ppt /
...
public slots:
	...
	void updatePosition() { updatePosition( MidiTime() ); }
