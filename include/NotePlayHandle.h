...
public:
	...
	NotePlayHandle( InstrumentTrack * instrumentTrack, const f_cnt_t offset,
	                const f_cnt_t frames, const Note & noteToPlay,
	                NotePlayHandle * parent = NULL, int midiEventChannel = -1,
	...
	/*! convenience function that returns offset for the first period and zero
	   otherwise, used by instruments to handle the offset: instruments have to
	   check this property and add the correct number of empty frames in the
	 ...
	 * deleted */
	virtual bool isFinished() const { return m_released && framesLeft() <= 0; }
	...
	/*! Returns whether note has a parent, e.g. is not part of an arpeggio or a
	 * chord */
...
public:
	static void init();
	static NotePlayHandle *
	acquire( InstrumentTrack * instrumentTrack, const f_cnt_t offset,
	         const f_cnt_t frames, const Note & noteToPlay,
	         NotePlayHandle * parent = NULL, int midiEventChannel = -1,
