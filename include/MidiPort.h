...
class MidiPort : public Model, public SerializingObject
{
	...
	mapPropertyFromModel( int, baseVelocity, setBaseVelocity,
	                      m_baseVelocityModel );
...
public:
	...
	MidiPort( const QString & name, MidiClient * client,
	          MidiEventProcessor * eventProcessor, Model * parent = NULL,
	          Mode mode = Disabled );
	virtual ~MidiPort();
	...
	int realOutputChannel() const { return outputChannel() - 1; }
	...
	virtual QString nodeName() const { return "midiport"; }
