...
// CombFeedback: a feedback comb filter - basically a simple delay line, makes a
// comb shape in the freq response CombFeedfwd: a feed-forward comb filter - an
// "inverted" comb filter, can be combined with CombFeedback to create a net
// allpass if negative gain is used CombFeedbackDualtap: same as CombFeedback
// but takes two delay values AllpassDelay: an allpass delay - combines feedback
// and feed-forward - has flat frequency response

// all classes are templated with channel count, any arbitrary channel count can
// be used for each fx

// Methods (for all classes):

// setDelay sets delay amount in frames. It's up to you to make this
// samplerate-agnostic. Fractions are allowed - linear interpolation is used to
// deal with them CombFeedbackDualTap is a special case: it requires 2 delay
// times
