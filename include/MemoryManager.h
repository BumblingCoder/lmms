...
const int MM_INITIAL_CHUNKS =
    1024 *
    1024; // how many chunks to allocate at startup - TODO: make configurable
const int MM_INCREMENT_CHUNKS =
    16 * 1024; // min. amount of chunks to increment at a time
...
struct MemoryPool
{
	...
	MemoryPool( size_t chunks ) : m_chunks( chunks )
	{
		m_free =
		    reinterpret_cast<char *>( MemoryHelper::alignedMalloc( chunks ) );
...
public:
	...
	static int
	extend( int chunks ); // returns index of created pool (for use by alloc)
	static void cleanup();
...
#define MM_ALLOC( type, count ) \
	reinterpret_cast<type *>( MemoryManager::alloc( sizeof( type ) * count ) )
