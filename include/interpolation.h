...
inline float cubicInterpolate( float v0, float v1, float v2, float v3, float x )
{
	...
	return ( v1 +
	         fastFmaf( 0.5f, frcu, x ) *
	             ( v2 - frcu * ( 1.0f / 6.0f ) -
	               fastFmaf( t1, ( 1.0f / 6.0f ), -v0 ) * ( 1.0f / 3.0f ) ) +
	         frsq * x * ( t1 * ( 1.0f / 6.0f ) - 0.5f * v2 ) +
...
inline float optimal4pInterpolate( float v0, float v1, float v2, float v3,
                                   float x )
{
	const float z = x - 0.5f;
...
inline float lagrangeInterpolate( float v0, float v1, float v2, float v3,
                                  float x )
{
	const float c0 = v1;
	const float c1 =
