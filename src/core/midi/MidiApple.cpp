...
void MidiApple::processOutEvent( const MidiEvent & event, const MidiTime & time,
                                 const MidiPort * port )
{
...
void MidiApple::subscribeReadablePort( MidiPort * port, const QString & dest,
                                       bool subscribe )
{
...
void MidiApple::subscribeWritablePort( MidiPort * port, const QString & dest,
                                       bool subscribe )
{
                                    ...
                                    void * srcConnRefCon )
{
	...
	for( uint32_t i = 0; i < pktlist->numPackets; ++i )
	{
		...
		if( continueSysEx )
		{
			...
			{
				// We would process the SysEx message here, as it is we're just
				// ignoring it
		...
		else
		{
			...
			while( iByte < nBytes )
			{
				...
				{
					// MIDI SysEx then we copy the rest of the message into the
	...
	for( ItemCount i = 0; i < deviceCount; ++i )
	{
		...
		for( ItemCount j = 0; j < entityCount; ++j )
		{
			...
			for( ItemCount k = 0; k < sourceCount; ++k )
			{
				...
				QString sourceName =
				    qsDeviceName + ":" + QString::fromUtf8( (char *) ( name ) );
                                   ...
                                   bool isIn )
{
	...
	CFStringRef inName =
	    CFStringCreateWithCString( 0, registeredName, kCFStringEncodingASCII );
	if( isIn )
	{
...
void MidiApple::NotifyCallback( const MIDINotification * message,
                                void * refCon )
{
