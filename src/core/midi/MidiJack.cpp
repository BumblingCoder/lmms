...
{
	//: When JACK(JACK Audio Connection Kit) disconnects, it will show the
	// following message (title)
	QString msg_short = MidiJack::tr( "JACK server down" );
	//: When JACK(JACK Audio Connection Kit) disconnects, it will show the
	// following message (dialog message)
	QString msg_long =
      ...
      m_quit( false )
{
	...
	else
	{
		...
		if( m_jackClient )
		{
			jack_set_process_callback( m_jackClient, JackMidiProcessCallback,
			                           this );
	...
	if( jackClient() )
	{
		...
		*/
		...
		m_input_port =
		    jack_port_register( jackClient(), "MIDI in", JACK_DEFAULT_MIDI_TYPE,
		                        JackPortIsInput, 0 );
