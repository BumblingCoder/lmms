...
const QString
    InstrumentSoundShaping::targetNames[InstrumentSoundShaping::NumTargets][3] =
                        ...
                        tr( "Q/Resonance" ) )
{
	...
	m_filterModel.addItem( tr( "SV LowPass" ),
	                       new PixmapLoader( "filter_lp" ) );
	m_filterModel.addItem( tr( "SV BandPass" ),
	                       new PixmapLoader( "filter_bp" ) );
	m_filterModel.addItem( tr( "SV HighPass" ),
	                       new PixmapLoader( "filter_hp" ) );
	m_filterModel.addItem( tr( "SV Notch" ),
	                       new PixmapLoader( "filter_notch" ) );
	m_filterModel.addItem( tr( "Fast Formant" ),
	                       new PixmapLoader( "filter_hp" ) );
...
float InstrumentSoundShaping::volumeLevel( NotePlayHandle * n,
                                           const f_cnt_t frame )
{
                                                 ...
                                                 NotePlayHandle * n )
{
	...
	f_cnt_t envReleaseBegin =
	    envTotalFrames - n->releaseFramesDone() + n->framesBeforeRelease();
	...
	if( m_filterEnabledModel.value() )
	{
		...
		{
			m_envLfoParameters[Resonance]->fillLevel( resBuffer, envTotalFrames,
			                                          envReleaseBegin, frames );
		}
		    ...
		    m_envLfoParameters[Resonance]->isUsed() )
		{
			for( fpp_t frame = 0; frame < frames; ++frame )
			{
				const float new_cut_val =
				    EnvelopeAndLfoParameters::expKnobVal( cutBuffer[frame] ) *
				        CUT_FREQ_MULTIPLIER +
				    fcv;

				const float new_res_val =
