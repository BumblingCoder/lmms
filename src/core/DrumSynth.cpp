...
void DrumSynth::UpdateEnv( int e, long t )
{
	...
	envData[e][ENV] =
	    envpts[e][1][(long) ( envData[e][PNT] + 0.f )] * 0.01f; // this level
	endEnv =
	    envpts[e][1][(long) ( envData[e][PNT] + 1.f )] * 0.01f; // next level
...
void DrumSynth::GetEnv( int env, const char * sec, const char * key,
                        const char * ini )
{
	char en[256], s[8];
	int i = 0, o = 0, ep = 0;
...
float DrumSynth::waveform( float ph, int form )
{
	...
	switch( form )
	{
		case 0:
			...
			break; // sine
		case 1:
			w = (float) fabs( 2.0f * (float) sin( fmod( 0.5f * ph, TwoPi ) ) ) -
			    1.f;
			break; // sine^2
		case 2:
...
int DrumSynth::GetPrivateProfileString( const char * sec, const char * key,
                                        const char * def, char * buffer,
...
int DrumSynth::GetPrivateProfileInt( const char * sec, const char * key,
...
float DrumSynth::GetPrivateProfileFloat( const char * sec, const char * key,
                                         float def, const char * file )
{
	char tmp[16];
	float f = 0.f;
...
int DrumSynth::GetDSFileSamples( const char * dsfile, int16_t *& wave,
                                 int channels, sample_rate_t Fs )
{
	// input file
	char sec[32];
	char ver[32];
	...
	if( TDroopRate > 0.f )
	{
		...
		TDroop = 1;
		F2 = F1 + ( ( F2 - F1 ) /
		            ( 1.f - (float) exp( TDroopRate * envData[1][MAX] ) ) );
		ddF = F1 - F2;
	}
	else
		ddF = F2 - F1;
	...
	if( ( GetPrivateProfileInt( sec, "Track1", 0, dsfile ) == 1 ) &&
	    ( TON == 1 ) )
	{
		OF1Sync = 1;
		OF1 = OF1 / F1;
	}
	if( ( GetPrivateProfileInt( sec, "Track2", 0, dsfile ) == 1 ) &&
	    ( TON == 1 ) )
	{
		OF2Sync = 1;
		OF2 = OF2 / F1;
	...
	BFStep2 =
	    1 + (int) ( ( 40 - ( BFStep2 / 2.5 ) ) / ( BQ2 + 1 + ( 1 * BF2 ) ) );
	...
	if( DiON == 1 )
	{
		...
		DGain = DAtten * DGain *
		        (float) powf( 10.0, 0.05 * GetPrivateProfileInt(
		                                       sec, "Clipping", 0, dsfile ) );
	}
	...
	*/
	...
	while( tpos < Length )
	{
		...
		if( TON == 1 ) // tone
		{
			...
			for( j = 0; j < 1200; j++ ) phi[j] = F2; // for overtone sync
		...
		for( t = tpos; t <= tplus; t++ )
		{
			...
			if( MainFilter == 1 ) // filter overtones
			{
				...
				MFtmp =
				    Ot + MFres * ( 1.f + ( 1.f / MFfb ) ) * ( MFin - MFout );
		...
		if( DiON == 1 ) // bit resolution
		{
			...
			for( j = 0; j < 1200; j += DStep ) // downsampling
			{
				...
				for( jj = DownStart; jj <= DownEnd; jj++ ) DF[jj] = DownAve;
			}
		}
		else
			for( j = 0; j < 1200; j++ ) DF[j] *= DGain;
