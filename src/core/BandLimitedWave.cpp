...
void BandLimitedWave::generateWaves()
{
	...
	else
	{
		for( i = 0; i <= MAXTBL; i++ )
		{
			...
			for( int ph = 0; ph < len; ph++ )
			{
				...
				do
				{
					hlen = static_cast<double>( len ) /
					       static_cast<double>( harm );
					const double amp = -1.0 / static_cast<double>( harm );
					// const double a2 = cos( om * harm * F_2PI );
					s += amp *
					     /*a2 **/ sin( static_cast<double>( ph * harm ) /
					                   static_cast<double>( len ) * F_2PI );
					harm++;
				} while( hlen > 2.0 );
			...
			{
				sample_t s =
				    s_waveforms[BandLimitedWave::BLSaw].sampleAt( i, ph ) / max;
				s_waveforms[BandLimitedWave::BLSaw].setSampleAt( i, ph, s );
			}
		}
	}
	...
	else
	{
		for( i = 0; i <= MAXTBL; i++ )
		{
			...
			{
				sample_t s =
				    s_waveforms[BandLimitedWave::BLSquare].sampleAt( i, ph ) /
				    max;
				s_waveforms[BandLimitedWave::BLSquare].setSampleAt( i, ph, s );
			}
		}
		...
		for( i = 0; i <= MAXTBL; i++ )
		{
			...
			for( int ph = 0; ph < len; ph++ )
			{
				...
				do
				{
					...
					// const double a2 = cos( om * harm * F_2PI );
					s += amp * /*a2 **/ sin(
					               ( static_cast<double>( ph * harm ) /
					                     static_cast<double>( len ) +
					                 ( ( harm + 1 ) % 4 == 0 ? 0.5 : 0.0 ) ) *
					               F_2PI );
					harm += 2;
				} while( hlen > 2.0 );
				s_waveforms[BandLimitedWave::BLTriangle].setSampleAt( i, ph,
				                                                      s );
				max = qMax( max, qAbs( s ) );
			}
			// normalize
			...
			{
				sample_t s =
				    s_waveforms[BandLimitedWave::BLTriangle].sampleAt( i, ph ) /
				    max;
				s_waveforms[BandLimitedWave::BLTriangle].setSampleAt( i, ph,
				                                                      s );
			}
	...
	else
	{
		for( i = 0; i <= MAXTBL; i++ )
		{
			...
			for( int ph = 0; ph < len; ph++ )
			{
				...
				const sample_t saw =
				    s_waveforms[BandLimitedWave::BLSaw].sampleAt( i, sawph );
				const sample_t tri =
				    s_waveforms[BandLimitedWave::BLTriangle].sampleAt( i, ph );
				s_waveforms[BandLimitedWave::BLMoog].setSampleAt(
				    i, ph, ( saw + tri ) * 0.5f );
			}
		}
	}
	...
	//
	// normally these are now provided with LMMS as pre-generated so we don't
	// have to do this, but I'm leaving the code here in case it's needed in the
	// future (maybe we add more waveforms or change the generation code or
	// mipmap format, etc.)

	/*

	// if you want to generate the files, you need to set the filenames and
	paths here -
	// can't use the usual wavetable directory here as it can require
	permissions on
	// some systems...
