/*
 * RingBuffer.cpp - an effective and flexible implementation of a ringbuffer for
 * LMMS
 *
...
void RingBuffer::writeAdding( sampleFrame * src, f_cnt_t offset,
                              f_cnt_t length )
{
	const f_cnt_t pos = ( m_position + offset ) % m_size;
	if( length == 0 )
...
void RingBuffer::writeAddingMultiplied( sampleFrame * src, f_cnt_t offset,
...
void RingBuffer::writeAddingMultiplied( sampleFrame * src, float offset,
                                        f_cnt_t length, float level )
{
...
void RingBuffer::writeSwappedAddingMultiplied( sampleFrame * src, float offset,
                                               f_cnt_t length, float level )
{
...
{
	float newsize =
	    static_cast<float>( ( m_size - m_fpp ) *
