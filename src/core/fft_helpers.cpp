...
int hanming( float * timebuffer, int length, WINDOWS type )
{
	...
	if( ( timebuffer == NULL ) || ( length <= 0 ) ) return -1;
	...
	for( i = 0; i < length; i++ )
	{
		timebuffer[i] =
		    timebuffer[i] *
		    ( alpha +
		      ( 1 - alpha ) * cos( 2 * F_PI * i / ( (float) length - 1.0 ) ) );
	}
...
int absspec( fftwf_complex * complex_buffer, float * absspec_buffer,
             int compl_length )
{
	int i;
...
/* build fewer subbands from many absolute spectrum values
   take care that - compressedbands[] array num_new elements long
     ...
     returns 0 on success, else -1          */
int compressbands( float * absspec_buffer, float * compressedband, int num_old,
                   int num_new, int bottom, int top )
{
	float ratio;
	int i, usefromold;
	float j;
...
int calc13octaveband31( float * absspec_buffer, float * subbands, int num_spec,
                        float max_frequency )
{
	static const int onethirdoctavecenterfr[] = {
	...
	{
		absspec_buffer[i] =
		    ( absspec_buffer[i] * absspec_buffer[i] ) / FFT_BUFFER_SIZE;
	...
	for( i = 0; i < 31; i++ )
	{
		...
		{
			fprintf( stderr,
			         "Error: calc13octaveband31() in fft_helpers.cpp line %d "
			         "failed.\n",
			         __LINE__ );
			return -1;
		}
