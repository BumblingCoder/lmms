...
inline void
loadTranslation( const QString & tname,
...
void printHelp()
{
	printf(
	    ...
	    "    --geometry <geometry>     Specify the size and position of the "
	    "main "
	    "window\n"
	    ...
	    "-p, --profile <out>           Dump profiling information to file "
	    "<out>\n"
	    ...
	    "    --allowroot               Bypass root user startup check (use "
	    "with "
	    "caution).\n"
...
int main( int argc, char ** argv )
{
	...
	for( int i = 1; i < argc; ++i )
	{
		...
		// Ignore, processed earlier
			...
			if( allowRoot )
			{
				printf( "\nOption \"--allowroot\" will be ignored on this "
				        "platform.\n\n" );
			}
#endif
		}
...
#define QT_TRANSLATIONS_DIR ConfigManager::inst()->localeDir()
	...
	else // otherwise, start the GUI
	{
			...
			mb.setText(
			    QString( "<html>"
			        ...
			        .arg(
			            ...
			            MainWindow::tr(
			                "Launch a default session and delete "
		...
		QString queuedFile =
		    static_cast<MainApplication *>( app )->queuedFile();
