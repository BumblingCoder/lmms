      ...
      m_stopSemaphore( 1 )
{
	...
	{
		printf( "Pa_OpenStream() failed with 44,1 KHz, trying again with 48 "
		        "KHz\n" );
		...
		for( fpp_t frame = 0; frame < min_len; ++frame )
		{
			for( ch_cnt_t chnl = 0; chnl < channels(); ++chnl )
			{
				( _outputBuffer + frame * channels() )[chnl] =
				    Mixer::clip( m_outBuf[frame][chnl] * master_gain );
			}
		}
...
int AudioPortAudio::_process_callback(
    const void * _inputBuffer, void * _outputBuffer,
    unsigned long _framesPerBuffer, const PaStreamCallbackTimeInfo * _timeInfo,
    PaStreamCallbackFlags _statusFlags, void * _arg )
{
	Q_UNUSED( _timeInfo );
