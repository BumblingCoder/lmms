                                ...
                                const fpp_t _frames, const float _master_gain )
{
	...
	int bytesWritten = lame_encode_buffer_interleaved_ieee_float(
	    m_lame, &interleavedDataBuffer[0], _frames, &encodingBuffer[0],
...
{
	// The documentation states that flush should have at least 7200 bytes. So
	// let's be generous.
	std::vector<unsigned char> encodingBuffer( 7200 * 4 );

	int bytesWritten = lame_encode_flush(
	    m_lame, &encodingBuffer[0], static_cast<int>( encodingBuffer.size() ) );
	assert( bytesWritten >= 0 );
...
bool AudioFileMP3::initEncoder()
{
	...
	OutputSettings::BitRateSettings bitRateSettings =
	    getOutputSettings().getBitRateSettings();
