...
static void stream_write_callback( pa_stream * s, size_t length,
                                   void * userdata )
{
	static_cast<AudioPulseAudio *>( userdata )
	    ->streamWriteCallback( s, length );
}
...
void AudioPulseAudio::stopProcessing() { stopProcessingThread( this ); }
...
void AudioPulseAudio::streamWriteCallback( pa_stream * s, size_t length )
{
	const fpp_t fpp = mixer()->framesPerPeriod();
	surroundSampleFrame * temp = new surroundSampleFrame[fpp];
	int_sample_t * pcmbuf = (int_sample_t *) pa_xmalloc(
	    fpp * channels() * sizeof( int_sample_t ) );
	...
	while( fd < length / 4 && m_quit == false )
	{
		...
		int bytes = convertToS16( temp, frames, mixer()->masterGain(), pcmbuf,
		                          m_convertEndian );
		if( bytes > 0 )
		{
