...
PeakController::PeakController( Model * _parent,
    ...
    : Controller( Controller::PeakController, _parent,
                  tr( "Peak Controller" ) ),
...
PeakController::~PeakController()
{
	...
	// not called  Therefore, we shouldn't call removeEffect() as it is not even
	// appended.  NB: Most XML setting are loaded on preview, except controller
	// fx.
...
void PeakController::updateValueBuffer()
{
	if( m_coeffNeedsUpdate )
	{
		...
		m_attackCoeff =
		    1.0f -
		    powf( 2.0f, -0.3f *
		                    ( 1.0f - m_peakEffect->attackModel()->value() ) *
		                    ratio );
		m_decayCoeff =
		    1.0f -
		    powf( 2.0f, -0.3f * ( 1.0f - m_peakEffect->decayModel()->value() ) *
		                    ratio );
