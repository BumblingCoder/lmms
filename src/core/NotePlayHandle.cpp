...
NotePlayHandle::NotePlayHandle( InstrumentTrack * instrumentTrack,
                                const f_cnt_t _offset, const f_cnt_t _frames,
                                const Note & n, NotePlayHandle * parent,
                                int midiEventChannel, Origin origin )
    : PlayHandle( TypeNotePlayHandle, _offset ),
      Note( n.length(), n.pos(), n.key(), n.getVolume(), n.getPanning(),
            n.detuning() ),
      m_pluginData( NULL ),
      m_filter( NULL ),
      ...
      m_midiChannel( midiEventChannel >= 0
                         ? midiEventChannel
                         : instrumentTrack->midiPort()->realOutputChannel() ),
      m_origin( origin ),
      m_frequencyNeedsUpdate( false )
{
	...
	if( hasParent() || !m_instrumentTrack->isArpeggioEnabled() )
	{
		...
		m_instrumentTrack->processOutEvent(
		    MidiEvent( MidiNoteOn, midiChannel(), midiKey(),
		               midiVelocity( baseVelocity ) ),
		    MidiTime::fromFrames( offset(), Engine::framesPerTick() ),
		    offset() );
	}

	if( m_instrumentTrack->instrument()->flags() &
...
void NotePlayHandle::setVolume( volume_t _volume )
{
	...
	m_instrumentTrack->processOutEvent(
	    MidiEvent( MidiKeyPressure, midiChannel(), midiKey(),
	               midiVelocity( baseVelocity ) ) );
}
...
void NotePlayHandle::setPanning( panning_t panning )
{
	...
	MidiEvent event( MidiMetaEvent, midiChannel(), midiKey(),
	                 panningToMidi( panning ) );
...
void NotePlayHandle::play( sampleFrame * _working_buffer )
{
	...
	f_cnt_t framesThisPeriod =
	    m_totalFramesPlayed == 0 ? Engine::mixer()->framesPerPeriod() - offset()
	    ...
	    m_totalFramesPlayed + framesThisPeriod > m_frames )
	{
		noteOff( m_totalFramesPlayed == 0
		             ? ( m_frames +
		                 offset() ) // if we have noteon and noteoff during
		                            // the same period, take offset in
		                            // account for release frame
		             : ( m_frames -
		                 m_totalFramesPlayed ) ); // otherwise, the offset is
		                                          // already negated and can
		                                          // be ignored
	}
	...
	if( framesLeft() > 0 )
	{
		...
		// ... also, they don't actually render the sound in NPH's, which is an
		// even better reason to skip...
		if( m_totalFramesPlayed == 0 &&
		    !( m_instrumentTrack->instrument()->flags() &
		       Instrument::IsSingleStreamed ) )
		{
	    ...
	    ( !instrumentTrack()->isSustainPedalPressed() || m_releaseStarted ) )
	{
		...
		{
			m_releaseFramesToDo =
			    m_releaseFramesDone + 2 * Engine::mixer()->framesPerPeriod();
		}
...
{
	return length() == _nph.length() && pos() == _nph.pos() &&
	       key() == _nph.key() && getVolume() == _nph.getVolume() &&
	       getPanning() == _nph.getPanning() &&
	       m_instrumentTrack == _nph.m_instrumentTrack &&
	       m_frames == _nph.m_frames && offset() == _nph.offset() &&
	       m_totalFramesPlayed == _nph.m_totalFramesPlayed &&
	       m_released == _nph.m_released && m_hasParent == _nph.m_hasParent &&
	       m_origBaseNote == _nph.m_origBaseNote && m_muted == _nph.m_muted &&
	       m_midiChannel == _nph.m_midiChannel && m_origin == _nph.m_origin;
}

void NotePlayHandle::updateFrequency()
{
	int mp = m_instrumentTrack->m_useMasterPitchModel.value()
	             ? Engine::getSong()->masterPitch()
	             : 0;
	const float pitch = ( key() - m_instrumentTrack->baseNoteModel()->value() +
	                      mp + m_baseDetuning->value() ) /
	                    12.0f;
	m_frequency = BaseFreq *
	              powf( 2.0f, pitch + m_instrumentTrack->pitchModel()->value() /
	                                      ( 100 * 12.0f ) );
...
void NotePlayHandle::processMidiTime( const MidiTime & time )
{
	...
	{
		const float v = detuning()->automationPattern()->valueAt(
...
NotePlayHandle * NotePlayHandleManager::acquire(
    InstrumentTrack * instrumentTrack, const f_cnt_t offset,
    const f_cnt_t frames, const Note & noteToPlay, NotePlayHandle * parent,
    int midiEventChannel, NotePlayHandle::Origin origin )
{
	...
	s_mutex.lockForRead();
	NotePlayHandle * nph =
	    s_available[s_availableIndex.fetchAndAddOrdered( -1 )];
	s_mutex.unlock();

	new( (void *) nph )
	    NotePlayHandle( instrumentTrack, offset, frames, noteToPlay, parent,
	                    midiEventChannel, origin );
	return nph;
}
