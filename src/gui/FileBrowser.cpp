      ...
      m_recurse( recurse )
{
	...
	QPushButton * reload_btn =
	    new QPushButton( embed::getIconPixmap( "reload" ), QString::null, ops );
	...
	// filter box.
	QShortcut * filterFocusShortcut = new QShortcut(
	    QKeySequence( QKeySequence::Find ), this, SLOT( giveFocusToFilter() ) );
...
void FileBrowser::addItems( const QString & path )
{
	     ...
	     it != files.constEnd(); ++it )
	{
		...
		if( cur_file[0] != '.' )
		{
			...
			{
				Directory * d =
				    dynamic_cast<Directory *>( m_l->topLevelItem( i ) );
	     ...
	     it != files.constEnd(); ++it )
	{
		...
		if( cur_file[0] != '.' )
		{
			...
			QList<QTreeWidgetItem *> existing =
			    m_l->findItems( cur_file, Qt::MatchFixedString );
			if( !existing.empty() )
			{
...
{
	if( m_mousePressed == true && ( m_pressPos - me->pos() ).manhattanLength() >
...
void FileBrowserTreeWidget::handleFile( FileItem * f, InstrumentTrack * it )
{
	...
	switch( f->handling() )
	{
		...
		case FileItem::LoadByPlugin:
		{
			...
			if( i == NULL || !i->descriptor()->supportsFileType( e ) )
			{
				i = it->loadInstrument(
                                              ...
                                              int column )
{
	...
	{
		InstrumentTrack * it = dynamic_cast<InstrumentTrack *>( Track::create(
		    Track::InstrumentTrack, Engine::getBBTrackContainer() ) );
		handleFile( f, it );
	}
}
...
void FileBrowserTreeWidget::sendToActiveInstrumentTrack( void )
{
	...
	QList<QMdiSubWindow *> pl = gui->mainWindow()->workspace()->subWindowList(
	...
	while( w.hasPrevious() )
	{
		InstrumentTrackWindow * itw =
		    dynamic_cast<InstrumentTrackWindow *>( w.previous()->widget() );
...
void FileItem::initPixmaps( void )
{
	...
	{
		s_projectFilePixmap =
		    new QPixmap( embed::getIconPixmap( "project_file", 16, 16 ) );
	}

	if( s_presetFilePixmap == NULL )
	{
		s_presetFilePixmap =
		    new QPixmap( embed::getIconPixmap( "preset_file", 16, 16 ) );
	}

	if( s_sampleFilePixmap == NULL )
	{
		s_sampleFilePixmap =
		    new QPixmap( embed::getIconPixmap( "sample_file", 16, 16 ) );
	}

	if( s_soundfontFilePixmap == NULL )
	{
		s_soundfontFilePixmap =
		    new QPixmap( embed::getIconPixmap( "soundfont_file", 16, 16 ) );
	}

	if( s_vstPluginFilePixmap == NULL )
	{
		s_vstPluginFilePixmap =
		    new QPixmap( embed::getIconPixmap( "vst_plugin_file", 16, 16 ) );
	}

	if( s_midiFilePixmap == NULL )
	{
		s_midiFilePixmap =
		    new QPixmap( embed::getIconPixmap( "midi_file", 16, 16 ) );
	}

	if( s_unknownFilePixmap == NULL )
	{
		s_unknownFilePixmap =
		    new QPixmap( embed::getIconPixmap( "unknown_file" ) );
	}
