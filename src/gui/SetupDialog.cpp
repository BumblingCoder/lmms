...
SetupDialog::SetupDialog( ConfigTabs _tab_to_open )
    : m_bufferSize( ConfigManager::inst()
      ...
      m_warnAfterSetup(
          !ConfigManager::inst()->value( "app", "nomsgaftersetup" ).toInt() ),
      m_displaydBFS(
      ...
      m_hqAudioDev(
          ConfigManager::inst()->value( "mixer", "hqaudio" ).toInt() ),
      ...
      m_ladDir(
          QDir::toNativeSeparators( ConfigManager::inst()->ladspaDir() ) ),
      ...
      m_enableAutoSave(
          ConfigManager::inst()->value( "ui", "enableautosave", "1" ).toInt() ),
      m_enableRunningAutoSave( ConfigManager::inst()
      ...
      m_compactTrackButtons(
          ConfigManager::inst()->value( "ui", "compacttrackbuttons" ).toInt() ),
      m_syncVSTPlugins(
          ...
          ConfigManager::inst()->value( "ui", "disableautoquit" ).toInt() )
{
	...
	LedCheckBox * enable_tooltips =
	    new LedCheckBox( tr( "Enable tooltips" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * restart_msg = new LedCheckBox(
	    tr( "Show restart warning after changing settings" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * dbfs =
	    new LedCheckBox( tr( "Display volume as dBFS " ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * mmpz =
	    new LedCheckBox( tr( "Compress project files per default" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * oneitw =
	    new LedCheckBox( tr( "One instrument track window mode" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * hqaudio =
	    new LedCheckBox( tr( "HQ-mode for output audio-device" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * compacttracks =
	    new LedCheckBox( tr( "Compact track buttons" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * syncVST =
	    new LedCheckBox( tr( "Sync VST plugins to host playback" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * noteLabels =
	    new LedCheckBox( tr( "Enable note labels in piano roll" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * displayWaveform =
	    new LedCheckBox( tr( "Enable waveform display by default" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * disableAutoquit = new LedCheckBox(
	    tr( "Keep effects running even without input" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * disableBackup = new LedCheckBox(
	    tr( "Create backup file when saving a project" ), misc_tw );
	labelNumber++;
	...
	LedCheckBox * openLastProject =
	    new LedCheckBox( tr( "Reopen last project on start" ), misc_tw );
	labelNumber++;
	...
	// working-dir
	TabWidget * lmms_wd_tw = new TabWidget(
	    tr( "LMMS working directory" ).toUpper(), pathSelectors );
	...
	QPushButton * workingdir_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", lmms_wd_tw );
	...
	// artwork-dir
	TabWidget * artwork_tw =
	    new TabWidget( tr( "Themes directory" ).toUpper(), pathSelectors );
	...
	QPushButton * artworkdir_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", artwork_tw );
	...
	TabWidget * backgroundArtwork_tw =
	    new TabWidget( tr( "Background artwork" ).toUpper(), paths );
	backgroundArtwork_tw->setFixedHeight( 48 );

	m_baLineEdit = new QLineEdit( m_backgroundArtwork, backgroundArtwork_tw );
	...
	QPushButton * backgroundartworkdir_select_btn =
	    new QPushButton( embed::getIconPixmap( "project_open", 16, 16 ), "",
	...
	// vst-dir
	TabWidget * vst_tw =
	    new TabWidget( tr( "VST-plugin directory" ).toUpper(), pathSelectors );
	...
	QPushButton * vstdir_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", vst_tw );
	...
	// gig-dir
	TabWidget * gig_tw =
	    new TabWidget( tr( "GIG directory" ).toUpper(), pathSelectors );
	...
	QPushButton * gigdir_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", gig_tw );
	...
	// sf2-dir
	TabWidget * sf2_tw =
	    new TabWidget( tr( "SF2 directory" ).toUpper(), pathSelectors );
	...
	QPushButton * sf2dir_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", sf2_tw );
	...
	// LADSPA-dir
	TabWidget * lad_tw =
	    new TabWidget( tr( "LADSPA plugin directories" ).toUpper(), paths );
	...
	QPushButton * laddir_select_btn = new QPushButton(
	    embed::getIconPixmap( "add_folder", 16, 16 ), "", lad_tw );
	...
	// STK-dir
	TabWidget * stk_tw =
	    new TabWidget( tr( "STK rawwave directory" ).toUpper(), paths );
	...
	QPushButton * stkdir_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", stk_tw );
	...
	// Soundfont
	TabWidget * sf_tw =
	    new TabWidget( tr( "Default Soundfont File" ).toUpper(), paths );
	...
	QPushButton * sf_select_btn = new QPushButton(
	    embed::getIconPixmap( "project_open", 16, 16 ), "", sf_tw );
	...
	m_autoSave = new LedCheckBox( tr( "Enable auto-save" ), auto_save_tw );
	...
	m_runningAutoSave =
	    new LedCheckBox( tr( "Allow auto-save while playing" ), auto_save_tw );
	...
	QPushButton * autoSaveResetBtn =
	    new QPushButton( embed::getIconPixmap( "reload" ), "", auto_save_tw );
	...
	LedCheckBox * smoothScroll =
	    new LedCheckBox( tr( "Smooth scroll in Song Editor" ), ui_fx_tw );
	...
	LedCheckBox * animAFP = new LedCheckBox(
	    tr( "Show playback cursor in AudioFileProcessor" ), ui_fx_tw );
	animAFP->move( 10, 40 );
	...
	QPushButton * audio_help_btn =
	    new QPushButton( embed::getIconPixmap( "help" ), "", audioiface_tw );
	...
	m_tabBar
	    ->addTab( performance, tr( "Performance settings" ), 2, false, true )
	...
	QPushButton * ok_btn =
	    new QPushButton( embed::getIconPixmap( "apply" ), tr( "OK" ), buttons );
	connect( ok_btn, SIGNAL( clicked() ), this, SLOT( accept() ) );

	QPushButton * cancel_btn = new QPushButton(
	    embed::getIconPixmap( "cancel" ), tr( "Cancel" ), buttons );
...
void SetupDialog::accept()
{
	...
	ConfigManager::inst()->setValue(
	    "mixer", "mididev", m_midiIfaceNames[m_midiInterfaces->currentText()] );
	...
	ConfigManager::inst()->setLADSPADir(
	    QDir::fromNativeSeparators( m_ladDir ) );
#ifdef LMMS_HAVE_FLUIDSYNTH
...
void SetupDialog::resetBufSize() { setBufferSize( DEFAULT_BUFFER_SIZE / 64 ); }
...
void SetupDialog::toggleToolTips( bool _enabled ) { m_toolTips = _enabled; }
...
void SetupDialog::toggleMMPZ( bool _enabled ) { m_MMPZ = _enabled; }
...
void SetupDialog::toggleHQAudioDev( bool _enabled ) { m_hqAudioDev = _enabled; }
...
void SetupDialog::toggleAnimateAFP( bool _enabled ) { m_animateAFP = _enabled; }

void SetupDialog::toggleNoteLabels( bool en ) { m_printNoteLabels = en; }

void SetupDialog::toggleDisplayWaveform( bool en ) { m_displayWaveform = en; }

void SetupDialog::toggleDisableAutoquit( bool en ) { m_disableAutoQuit = en; }
...
void SetupDialog::setWorkingDir( const QString & _wd ) { m_workingDir = _wd; }
...
void SetupDialog::setVSTDir( const QString & _vd ) { m_vstDir = _vd; }

void SetupDialog::setGIGDir( const QString & _gd ) { m_gigDir = _gd; }

void SetupDialog::setSF2Dir( const QString & _sfd ) { m_sf2Dir = _sfd; }
...
void SetupDialog::setArtworkDir( const QString & _ad ) { m_artworkDir = _ad; }
...
void SetupDialog::openBackgroundArtwork()
{
	...
	QString dir =
	    ( m_backgroundArtwork.isEmpty() ) ? m_artworkDir : m_backgroundArtwork;
	QString new_file =
	    FileDialog::getOpenFileName( this, tr( "Choose background artwork" ),
	                                 dir, "Image Files (" + fileTypes + ")" );
...
void SetupDialog::setLADSPADir( const QString & _fd ) { m_ladDir = _fd; }
...
void SetupDialog::setAutoSaveInterval( int value )
{
	...
	minutes =
	    QString( "%1 %2" ).arg( QString::number( m_saveInterval ), minutes );
