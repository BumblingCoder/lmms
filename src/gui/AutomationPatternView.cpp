...
void AutomationPatternView::resetName() { m_pat->setName( QString::null ); }
...
void AutomationPatternView::disconnectObject( QAction * _a )
{
	...
	if( j && dynamic_cast<AutomatableModel *>( j ) )
	{
		...
		m_pat->m_objects.erase(
		    qFind( m_pat->m_objects.begin(), m_pat->m_objects.end(),
...
void AutomationPatternView::constructContextMenu( QMenu * _cm )
{
	...
	_cm->addAction( embed::getIconPixmap( "edit_erase" ), tr( "Clear" ), m_pat,
	                SLOT( clear() ) );
	_cm->addSeparator();
	...
	_cm->addAction( embed::getIconPixmap( "record" ), tr( "Set/clear record" ),
	...
	_cm->addAction( embed::getIconPixmap( "flip_x" ),
	                tr( "Flip Horizontally (Visible)" ), this,
	                SLOT( flipX() ) );
	if( !m_pat->m_objects.isEmpty() )
	{
		_cm->addSeparator();
		QMenu * m = new QMenu(
		    tr( "%1 Connections" ).arg( m_pat->m_objects.count() ), _cm );
...
void AutomationPatternView::paintEvent( QPaintEvent * )
{
	     ...
	     it != m_pat->getTimeMap().end(); ++it )
	{
		...
		path.lineTo( x_base + ( ( it + 1 ).key() ) * ppTick,
		             values[( it + 1 ).key() - 1 - it.key()] );
...
void AutomationPatternView::scaleTimemapToFit( float oldMin, float oldMax )
{
	     ...
	     it != m_pat->m_timeMap.end(); ++it )
	{
		...
		*it = ( *it - oldMin ) * ( newMax - newMin ) / ( oldMax - oldMin ) +
		      newMin;
	}
