...
void TempoSyncKnob::modelChanged()
{
	...
	connect( this, SIGNAL( sliderMoved( float ) ), model(),
	         SLOT( disableSync() ) );
	updateDescAndIcon();
}
...
void TempoSyncKnob::contextMenuEvent( QContextMenuEvent * )
{
	...
	if( limit / 8.0f <= model()->maxValue() )
	{
		...
		syncMenu
		    ->addAction( embed::getIconPixmap( "note_none" ), tr( "No Sync" ) )
		    ->setData( (int) TempoSyncKnobModel::SyncNone );
		if( limit / 0.125f <= model()->maxValue() )
		{
			syncMenu
			    ->addAction( embed::getIconPixmap( "note_double_whole" ),
			                 tr( "Eight beats" ) )
		...
		{
			syncMenu
			    ->addAction( embed::getIconPixmap( "note_whole" ),
			                 tr( "Whole note" ) )
		...
		{
			syncMenu
			    ->addAction( embed::getIconPixmap( "note_half" ),
			                 tr( "Half note" ) )
		...
		{
			syncMenu
			    ->addAction( embed::getIconPixmap( "note_quarter" ),
			                 tr( "Quarter note" ) )
		...
		{
			syncMenu
			    ->addAction( embed::getIconPixmap( "note_eighth" ),
			                 tr( "8th note" ) )
		...
		{
			syncMenu
			    ->addAction( embed::getIconPixmap( "note_sixteenth" ),
			                 tr( "16th note" ) )
			    ->setData( (int) TempoSyncKnobModel::SyncSixteenthNote );
		}
		syncMenu
		    ->addAction( embed::getIconPixmap( "note_thirtysecond" ),
		                 tr( "32nd note" ) )
		    ->setData( (int) TempoSyncKnobModel::SyncThirtysecondNote );
		syncMenu
		    ->addAction( embed::getIconPixmap( "dont_know" ), tr( "Custom..." ),
		...
		switch( model()->m_tempoSyncMode )
		{
			case TempoSyncKnobModel::SyncCustom:
				m_tempoSyncDescription =
				    tr( "Custom " ) + "(" +
				    QString::number(
				        model()->m_custom.numeratorModel().value() ) +
				    "/" +
				    QString::number(
				        model()->m_custom.denominatorModel().value() ) +
				    ")";
				break;
