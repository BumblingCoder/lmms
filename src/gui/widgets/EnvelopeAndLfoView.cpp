    ...
    : QWidget( _parent ), ModelView( NULL, this ), m_params( NULL )
{
	...
	sin_lfo_btn->setInactiveGraphic(
	    embed::getIconPixmap( "sin_wave_inactive" ) );
	sin_lfo_btn->setWhatsThis( tr( "Click here for a sine-wave." ) );

	PixmapButton * triangle_lfo_btn = new PixmapButton( this, NULL );
	triangle_lfo_btn->move( LFO_SHAPES_X + 15, LFO_SHAPES_Y );
	triangle_lfo_btn->setActiveGraphic(
	    embed::getIconPixmap( "triangle_wave_active" ) );
	triangle_lfo_btn->setInactiveGraphic(
	    embed::getIconPixmap( "triangle_wave_inactive" ) );
	triangle_lfo_btn->setWhatsThis( tr( "Click here for a triangle-wave." ) );
	...
	m_userLfoBtn->setInactiveGraphic(
	    embed::getIconPixmap( "usr_wave_inactive" ) );
	m_userLfoBtn->setWhatsThis( tr( "Click here for a user-defined wave. "
	                                "Afterwards, drag an according sample-"
	                                "file onto the LFO graph." ) );

	connect( m_userLfoBtn, SIGNAL( toggled( bool ) ), this,
	         SLOT( lfoUserWaveChanged() ) );

	PixmapButton * random_lfo_btn = new PixmapButton( this, NULL );
	random_lfo_btn->move( LFO_SHAPES_X + 60, LFO_SHAPES_Y );
	random_lfo_btn->setActiveGraphic(
	    embed::getIconPixmap( "random_wave_active" ) );
	random_lfo_btn->setInactiveGraphic(
	    embed::getIconPixmap( "random_wave_inactive" ) );
	random_lfo_btn->setWhatsThis( tr( "Click here for random wave." ) );
...
void EnvelopeAndLfoView::paintEvent( QPaintEvent * )
{
	...
	int x1 =
	    static_cast<int>( m_predelayKnob->value<float>() * TIME_UNIT_WIDTH );
	int x2 =
	    x1 + static_cast<int>( m_attackKnob->value<float>() * TIME_UNIT_WIDTH );
	int x3 =
	    x2 + static_cast<int>( m_holdKnob->value<float>() * TIME_UNIT_WIDTH );
	int x4 = x3 + static_cast<int>( ( m_decayKnob->value<float>() *
	                                  ( 1 - m_sustainKnob->value<float>() ) ) *
	                                TIME_UNIT_WIDTH );
	int x5 = x4 + static_cast<int>( m_releaseKnob->value<float>() *
	                                TIME_UNIT_WIDTH );
	...
	const float frames_for_graph =
	    SECS_PER_LFO_OSCILLATION * Engine::mixer()->baseSampleRate() / 10;

	const float lfo_gray_amount =
	    1.0f - fabsf( m_lfoAmountKnob->value<float>() );
	...
	for( int x = 0; x <= LFO_GRAPH_W; ++x )
	{
		...
		{
			float phase =
			    ( cur_sample -= m_params->m_lfoPredelayFrames ) / osc_frames;
			switch( m_params->m_lfoWaveModel.value() )
			{
				...
				case EnvelopeAndLfoParameters::RandomWave:
					if( x % (int) ( 900 * m_lfoSpeedKnob->value<float>() +
					                1 ) ==
					    0 )
					{
	...
	int ms_per_osc = static_cast<int>(
	    SECS_PER_LFO_OSCILLATION * m_lfoSpeedKnob->value<float>() * 1000.0f );
