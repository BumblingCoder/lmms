/*
 * LeftRightNav.cpp - side-by-side left-facing and right-facing arrows for
      ...
      m_rightBtn( this, tr( "Next" ) )
{
	...
	m_rightBtn.setActiveGraphic(
	    embed::getIconPixmap( "stepper-right-press" ) );
	...
	// AutomatableButton's right click menu (contains irrelevant options like
...
void LeftRightNav::setShortcuts( const QKeySequence & leftShortcut,
                                 const QKeySequence & rightShortcut )
{
	m_leftBtn.setShortcut( leftShortcut );
	m_rightBtn.setShortcut( rightShortcut );

	ToolTip::add( &m_leftBtn,
	              tr( "Previous (%1)" ).arg( leftShortcut.toString() ) );
	ToolTip::add( &m_rightBtn,
	              tr( "Next (%1)" ).arg( rightShortcut.toString() ) );
}