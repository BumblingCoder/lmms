...
SendButtonIndicator::SendButtonIndicator( QWidget * _parent, FxLine * _owner,
                                          FxMixerView * _mv )
    : QLabel( _parent ), m_parent( _owner ), m_mv( _mv )
{
	if( !s_qpmOff )
	{
		s_qpmOff =
		    new QPixmap( embed::getIconPixmap( "mixer_send_off", 29, 20 ) );
	}

	if( !s_qpmOn )
	{
		s_qpmOn =
		    new QPixmap( embed::getIconPixmap( "mixer_send_on", 29, 20 ) );
	}
...
{
	FxMixer * mix = Engine::fxMixer();
	return mix->channelSendModel( m_mv->currentFxLine()->channelIndex(),
	                              m_parent->channelIndex() );
}
