...
#define DEFAULT_LCDWIDGET_INITIALIZER_LIST                      \
	QWidget( parent ), m_label(), m_textColor( 255, 255, 255 ), \
	    m_textShadowColor( 64, 64, 64 )

LcdWidget::LcdWidget( QWidget * parent, const QString & name )
    : DEFAULT_LCDWIDGET_INITIALIZER_LIST, m_numDigits( 1 )
{
	initUi( name );
}

LcdWidget::LcdWidget( int numDigits, QWidget * parent, const QString & name )
    : DEFAULT_LCDWIDGET_INITIALIZER_LIST, m_numDigits( numDigits )
{
	initUi( name );
}

LcdWidget::LcdWidget( int numDigits, const QString & style, QWidget * parent,
                      const QString & name )
    : DEFAULT_LCDWIDGET_INITIALIZER_LIST, m_numDigits( numDigits )
{
	initUi( name, style );
...
void LcdWidget::paintEvent( QPaintEvent * )
{
	...
	{
		p.drawPixmap(
		    cellRect, *m_lcdPixmap,
		    QRect( QPoint( 10 * m_cellWidth, isEnabled() ? 0 : m_cellHeight ),
		           cellSize ) );
	...
	for( int i = 0; i < m_display.length(); i++ )
	{
		...
		p.drawPixmap(
		    cellRect, *m_lcdPixmap,
		    QRect( QPoint( val * m_cellWidth, isEnabled() ? 0 : m_cellHeight ),
		           cellSize ) );
		p.translate( m_cellWidth, 0 );
	}
	...
	opt.rect = QRect(
	    0, 0, m_cellWidth * m_numDigits + ( margin + m_marginWidth ) * 2 - 1,
...
void LcdWidget::updateSize()
{
	...
	if( m_label.isEmpty() )
	{
		setFixedSize( m_cellWidth * m_numDigits +
...
void LcdWidget::initUi( const QString & name, const QString & style )
{
	...
	// m_lcdPixmap = new QPixmap( embed::getIconPixmap( QString( "lcd_" + style
	// ).toUtf8().constData() ) );  m_lcdPixmap = new QPixmap(
