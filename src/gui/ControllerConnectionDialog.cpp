      ...
      m_midiController( NULL )
{
	...
	connect( m_midiGroupBox->model(), SIGNAL( dataChanged() ), this,
	         SLOT( midiToggled() ) );
	...
	connect( m_userGroupBox->model(), SIGNAL( dataChanged() ), this,
	         SLOT( userToggled() ) );
	...
	connect( select_btn, SIGNAL( clicked() ), this,
	         SLOT( selectController() ) );
...
void ControllerConnectionDialog::midiToggled()
{
	...
	if( enabled != 0 )
	{
		...
		if( !m_midiController )
		{
			...
			connect( m_midiController, SIGNAL( valueChanged() ), this,
			         SLOT( midiValueChanged() ) );
		}
	}
