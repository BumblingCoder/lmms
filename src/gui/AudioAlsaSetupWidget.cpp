      ...
      m_selectedDevice( -1 )
{
	...
	for( size_t i = 0; i < m_deviceInfos.size(); ++i )
	{
		...
		m_deviceComboBox->addItem( comboBoxText,
		                           QVariant( static_cast<uint>( i ) ) );
	...
	connect( m_deviceComboBox, SIGNAL( currentIndexChanged( int ) ),
...
void AudioAlsaSetupWidget::saveSettings()
{
	...
	{
		AudioAlsa::DeviceInfo const & selectedDevice =
		    m_deviceInfos[m_selectedDevice];
	...
	ConfigManager::inst()->setValue(
	    "audioalsa", "channels", QString::number( m_channels->value<int>() ) );
}
