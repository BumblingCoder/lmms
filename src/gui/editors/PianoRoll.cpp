...
static QString s_noteStrings[12] = {"C",  "C#", "D",  "D#", "E",  "F",
                                    "F#", "G",  "G#", "A",  "A#", "B"};
...
const QVector<double> PianoRoll::m_zoomLevels = {0.125f, 0.25f, 0.5f, 1.0f,
                                                 2.0f,   4.0f,  8.0f};
      ...
      m_backgroundShade( 0, 0, 0 )
{
	...
	if( s_whiteKeySmallPressedPm == NULL )
	{
		s_whiteKeySmallPressedPm =
		    new QPixmap( embed::getIconPixmap( "pr_white_key_small_pressed" ) );
	}
	...
	if( s_whiteKeyBigPressedPm == NULL )
	{
		s_whiteKeyBigPressedPm =
		    new QPixmap( embed::getIconPixmap( "pr_white_key_big_pressed" ) );
	}
...
void PianoRoll::showTextFloat( const QString & text, const QPoint & pos,
                               int timeout )
{
...
void PianoRoll::showVolTextFloat( volume_t vol, const QPoint & pos,
                                  int timeout )
{
...
void PianoRoll::showPanTextFloat( panning_t pan, const QPoint & pos,
                                  int timeout )
{
	QString text;
	if( pan < 0 )
...
QColor PianoRoll::barLineColor() const { return m_barLineColor; }

void PianoRoll::setBarLineColor( const QColor & c ) { m_barLineColor = c; }

QColor PianoRoll::beatLineColor() const { return m_beatLineColor; }

void PianoRoll::setBeatLineColor( const QColor & c ) { m_beatLineColor = c; }

QColor PianoRoll::lineColor() const { return m_lineColor; }

void PianoRoll::setLineColor( const QColor & c ) { m_lineColor = c; }

QColor PianoRoll::noteModeColor() const { return m_noteModeColor; }

void PianoRoll::setNoteModeColor( const QColor & c ) { m_noteModeColor = c; }

QColor PianoRoll::noteColor() const { return m_noteColor; }

void PianoRoll::setNoteColor( const QColor & c ) { m_noteColor = c; }

QColor PianoRoll::barColor() const { return m_barColor; }

void PianoRoll::setBarColor( const QColor & c ) { m_barColor = c; }

QColor PianoRoll::selectedNoteColor() const { return m_selectedNoteColor; }
...
QColor PianoRoll::textColor() const { return m_textColor; }

void PianoRoll::setTextColor( const QColor & c ) { m_textColor = c; }

QColor PianoRoll::textColorLight() const { return m_textColorLight; }

void PianoRoll::setTextColorLight( const QColor & c ) { m_textColorLight = c; }

QColor PianoRoll::textShadow() const { return m_textShadow; }

void PianoRoll::setTextShadow( const QColor & c ) { m_textShadow = c; }

QColor PianoRoll::markedSemitoneColor() const { return m_markedSemitoneColor; }
...
int PianoRoll::noteOpacity() const { return m_noteOpacity; }

void PianoRoll::setNoteOpacity( const int i ) { m_noteOpacity = i; }

bool PianoRoll::noteBorders() const { return m_noteBorders; }

void PianoRoll::setNoteBorders( const bool b ) { m_noteBorders = b; }

QColor PianoRoll::backgroundShade() const { return m_backgroundShade; }
...
void PianoRoll::drawNoteRect( QPainter & p, int x, int y, int width,
                              const Note * n, const QColor & noteCol,
                              ...
                              const bool borders )
{
	...
	int volVal = qMin(
	    255, 100 + (int) ( ( (float) ( n->getVolume() - MinVolume ) ) /
...
void PianoRoll::keyPressEvent( QKeyEvent * ke )
{
	...
	switch( ke->key() )
	{
		...
		case Qt::Key_Left:
		{
			...
			{
				// switch to editing a pattern adjacent to this one in the song
				// editor
...
int PianoRoll::noteEditLeft() const { return WHITE_KEY_WIDTH; }
	...
	if( me->y() > PR_TOP_MARGIN )
	{
				...
				else
				{
					...
					if( !is_new_note && me->modifiers() & Qt::ShiftModifier )
					{
						...
						if( newNotes.size() != 0 )
						{
							...
							{
								Note * newNote =
								    m_pattern->addNote( newNotes[i], false );
		...
		else
		{
			if( me->buttons() == Qt::LeftButton )
			{
				...
				// area
				m_noteEditMode =
				    ( NoteEditMode )( ( (int) m_noteEditMode ) + 1 );
	    ...
	    me->y() > noteEditTop() && me->y() < noteEditBottom() )
	{
		...
		if( nv.size() > 0 )
		{
			...
			if( nv.size() > 1 )
			{
				...
				// ... then remove all notes from the vector that aren't on the
				// same exact time
...
{
	if( m_selectStartTick == 0 && m_selectedTick == 0 &&
	    m_selectStartKey == 0 && m_selectedKeys == 0 )
	{
...
void PianoRoll::mouseMoveEvent( QMouseEvent * me )
{
	...
	else if( m_action == ActionResizeNoteEditArea )
	{
		...
		m_notesEditHeight =
		    tLimit<int>( m_oldNotesEditHeight - ( me->y() - m_moveStartY ),
	...
	if( me->y() > PR_TOP_MARGIN || m_action != ActionNone )
	{
		             ...
		             me->modifiers() & Qt::ShiftModifier ) ) )
		{
			...
			for( int i = 0; i < notes.size(); ++i )
			{
				...
				// If note is the one under the cursor or is selected when alt
				// is not pressed
		...
		else if( me->buttons() == Qt::NoButton && m_editMode == ModeDraw )
		{
			...
			if( it != notes.begin() - 1 )
			{
				...
				bool atTail =
				    note->length() > 0 && x > noteRightX - RESIZE_AREA_WIDTH;
	...
	else
	{
		    ...
		    m_action == ActionSelectNotes )
		{
			...
			{
				QCursor::setPos(
				    mapToGlobal( QPoint( me->x(), PR_TOP_MARGIN ) ) );
				m_topBottomScroll->setValue( m_topBottomScroll->value() - 1 );
...
void PianoRoll::dragNotes( int x, int y, bool alt, bool shift, bool ctrl )
{
	...
	else if( m_action == ActionResizeNote )
	{
		...
		if( shift )
		{
			...
			// This factor is such that the endpoint of the note whose handle is
			// being dragged should lie under the cursor. first, determine the
			// start-point of the left-most selected note:
				    ...
				    ( posteriorNote == nullptr ||
				      note->oldPos().getTicks() + note->oldLength().getTicks() >
				          posteriorNote->oldPos().getTicks() +
				              posteriorNote->oldLength().getTicks() ) )
				{
					posteriorNote = note;
				}
			}
			...
			// Calculate factor by which to scale the start-point and end-point
			...
			for( Note * note : notes )
			{
				if( note->selected() )
				{
					...
					int newStart = stretchStartTick +
					               scaleFactor * ( note->oldPos().getTicks() -
					                               stretchStartTick );
					...
					if( !alt )
					{
						...
						endDiff =
						    floor( endDiff / quantization() ) * quantization();
			...
			if( ctrl || selectionCount() == 1 )
			{
				...
				for( Note * note : notes )
				{
					...
					{
						int newStart =
						    note->pos().getTicks() + posteriorDeltaThisFrame;
						note->setPos( MidiTime( newStart ) );
					}
				}
			}
...
void PianoRoll::paintEvent( QPaintEvent * pe )
{
	...
	switch( prKeyOrder[m_startKey % KeysPerOctave] )
	{
		...
		case PR_WHITE_KEY_SMALL:
			if( prKeyOrder[( ( m_startKey + 1 ) % KeysPerOctave )] !=
			    PR_BLACK_KEY )
			{
				y_offset = KEY_LINE_HEIGHT / 2;
			}
			        ...
			        key ) )
			{
				p.drawPixmap(
				    ...
				    y - ( first_white_key_height - WHITE_KEY_SMALL_HEIGHT ) -
				        WHITE_KEY_SMALL_HEIGHT / 2 - 1 - BLACK_KEY_HEIGHT,
				    *s_blackKeyPressedPm );
			}
			else
			{
...
void PianoRoll::setEditMode( int mode ) { m_editMode = (EditModes) mode; }
...
void PianoRoll::copyToClipboard( const NoteVector & notes ) const
{
	...
	clip_content->setData( Clipboard::mimeType(),
	                       dataFile.toString().toUtf8() );
...
PianoRollWindow::PianoRollWindow() : Editor( true ), m_editor( new PianoRoll() )
{
	...
	m_recordAccompanyAction->setToolTip( tr(
	    "Record notes from MIDI-device/channel-piano while playing song or BB "
	    "track" ) );
	...
	QAction * pasteAction =
	    new QAction( embed::getIconPixmap( "edit_paste" ),
	                 tr( "Paste notes from clipboard (%1+V)" )
	                     .arg(
#ifdef LMMS_BUILD_APPLE
	                         "⌘" ),
	                 this );
	...
	m_chordComboBox->setWhatsThis( tr(
	    ...
	    "After you have selected a chord, click anywhere to place the chord, "
	    "and right "
	    "click on the virtual keyboard to open context menu and highlight the "
	    "chord. "
...
void PianoRollWindow::record() { m_editor->record(); }

void PianoRollWindow::recordAccompany() { m_editor->recordAccompany(); }
