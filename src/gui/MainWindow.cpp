...
void MainWindow::finalize()
{
	...
	project_menu->addAction( embed::getIconPixmap( "project_new" ),
	                         tr( "&New" ), this, SLOT( createNewProject() ),
	                         QKeySequence::New );
	...
	m_recentlyOpenedProjectsMenu =
	    project_menu->addMenu( embed::getIconPixmap( "project_open_recent" ),
	...
	project_menu->addAction( embed::getIconPixmap( "project_export" ),
	                         tr( "E&xport..." ), Engine::getSong(),
	                         SLOT( exportProject() ), Qt::CTRL + Qt::Key_E );
	project_menu->addAction( embed::getIconPixmap( "project_export" ),
	                         tr( "E&xport Tracks..." ), Engine::getSong(),
	                         SLOT( exportProjectTracks() ),
	                         Qt::CTRL + Qt::SHIFT + Qt::Key_E );

	project_menu->addAction(
	    embed::getIconPixmap( "midi_file" ), tr( "Export &MIDI..." ),
	    Engine::getSong(), SLOT( exportProjectMidi() ), Qt::CTRL + Qt::Key_M );
...
// https://bugreports.qt.io/browse/QTBUG-40071
	...
	m_undoAction =
	    edit_menu->addAction( embed::getIconPixmap( "edit_undo" ), tr( "Undo" ),
	                          this, SLOT( undo() ), QKeySequence::Undo );
	m_redoAction =
	    edit_menu->addAction( embed::getIconPixmap( "edit_redo" ), tr( "Redo" ),
	...
	if( true )
	{
		help_menu->addAction( embed::getIconPixmap( "help" ),
		                      tr( "Online Help" ), this, SLOT( browseHelp() ) );
	}
	else
	{
...
// https://bugreports.qt.io/browse/QTBUG-40071
	...
	ToolButton * project_new = new ToolButton(
	    embed::getIconPixmap( "project_new" ), tr( "Create new project" ), this,
	    SLOT( createNewProject() ), m_toolBar );

	ToolButton * project_new_from_template =
	    new ToolButton( embed::getIconPixmap( "project_new_from_template" ),
	...
	m_metronomeToggle = new ToolButton(
	    embed::getIconPixmap( "metronome" ), tr( "Toggle metronome" ), this,
	...
	ToolButton * bb_editor_window =
	    new ToolButton( embed::getIconPixmap( "bb_track_btn" ),
	                    tr( "Show/hide Beat+Bassline Editor" ) + " (F6)", this,
	...
	ToolButton * piano_roll_window = new ToolButton(
	    embed::getIconPixmap( "piano" ), tr( "Show/hide Piano-Roll" ) + " (F7)",
	    this, SLOT( togglePianoRollWin() ), m_toolBar );
	...
	ToolButton * automation_editor_window =
	...
	for( QWidget * widget :
	     std::list<QWidget *>{gui->automationEditor(), gui->getBBEditor(),
	                          gui->pianoRoll(), gui->songEditor()} )
	{
...
{
	m_toolBarLayout->setColumnMinimumWidth( m_toolBarLayout->columnCount() + 7,
	                                        _size );
}

SubWindow * MainWindow::addWindowedWidget( QWidget * w,
                                           Qt::WindowFlags windowFlags )
{
	// wrap the widget in our own *custom* window that patches some errors in
	// QMdiSubWindow
...
{
	// If our widget is the main content of a window (e.g. piano roll, FxMixer,
	// etc), we really care about the position of the *window* - not the
	// position of the widget within its window
	...
	// getTrueNormalGeometry() method that performs the same as normalGeometry,
	// but isn't broken on X11 ( see https://bugreports.qt.io/browse/QTBUG-256 )
...
void MainWindow::restoreWidgetState( QWidget * _w, const QDomElement & _de )
{
	...
	if( _de.hasAttribute( "visible" ) && !r.isNull() )
	{
		...
		// first restore the window, as attempting to resize a maximized window
		// causes graphics glitching
...
void MainWindow::enterWhatsThisMode() { QWhatsThis::enterWhatsThisMode(); }
...
bool MainWindow::saveProjectAs()
{
	...
	QString suffix =
	    ConfigManager::inst()->value( "app", "nommpz" ).toInt() == 0 ? "mmpz"
	                                                                 : "mmp";
...
{
	if( !Engine::getSong()->isExporting() && !QApplication::mouseButtons() &&
	    ( ConfigManager::inst()
	          ->value( "ui", "enablerunningautosave" )
	          .toInt() ||
