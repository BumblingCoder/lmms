...
MidiTime Pattern::beatPatternLength() const
{
	...
	{
		max_length =
		    m_steps * MidiTime::ticksPerTact() / MidiTime::stepsPerTact();
	}
      ...
      m_paintPixmap()
{
	...
	{
		s_stepBtnOn200 =
		    new QPixmap( embed::getIconPixmap( "step_btn_on_200" ) );
	}
...
void PatternView::update()
{
	...
	{
		ToolTip::add( this, tr( "use mouse wheel to set velocity of a step" ) );
	}
	else
	{
		ToolTip::add( this, tr( "double-click to open in Piano Roll" ) );
	}
...
void PatternView::constructContextMenu( QMenu * _cm )
{
	...
	_cm->addAction( embed::getIconPixmap( "reload" ), tr( "Reset name" ), this,
	                SLOT( resetName() ) );
	_cm->addAction( embed::getIconPixmap( "edit_rename" ), tr( "Change name" ),
...
void PatternView::mousePressEvent( QMouseEvent * _me )
{
...

	{
		...
		{
			qDebug(
			    "Something went wrong in pattern.cpp: step %d doesn't exist in "
			    "pattern!",
			    step );
			return;
		}
...
void PatternView::paintEvent( QPaintEvent * )
{
	...
	m_paintPixmap =
	    m_paintPixmap.isNull() == true || m_paintPixmap.size() != size()
	        ? QPixmap( size() )
	        : m_paintPixmap;
	...
	if( m_pat->m_patternType == Pattern::MelodyPattern )
	{
		if( m_pat->m_notes.size() > 0 )
		{
			...
			if( total_notes > 0 )
			{
				...
				const int keyrange = qMax(
				    qMax( max_key - central_key, central_key - min_key ), 1 );
				     ...
				     it != m_pat->m_notes.end(); ++it )
				{
					...
					const float y_key =
					    ( float( central_key - ( *it )->key() ) / keyrange +
					      1.0f ) /
					    2;
					// multiply that by pattern height
					const int y_pos =
					    static_cast<int>( TCO_BORDER_WIDTH + y_key * ht ) + 1;

					// debug code
					// if( ( *it )->length() > 0 ) qDebug( "key %d, central_key
					// %d, y_key %f, y_pos %d", ( *it )->key(), central_key,
					// y_key, y_pos );

					// check that note isn't out of bounds, and has a length
					if( y_pos >= TCO_BORDER_WIDTH && y_pos <= max_ht )
					{
						// calculate start and end x-coords of the line to be
						// drawn
						...
						if( x1 < width() - TCO_BORDER_WIDTH )
							p.drawLine( x1, y_pos,
							            qMin( x2, width() - TCO_BORDER_WIDTH ),
							            y_pos );
					}
				}
			}
	                          ...
	                          m_pat->m_steps != MidiTime::stepsPerTact() ) )
	{
		...
		stepon0 = s_stepBtnOn0->scaled( w / steps, s_stepBtnOn0->height(),
		                                Qt::IgnoreAspectRatio,
		                                Qt::SmoothTransformation );
		stepon200 = s_stepBtnOn200->scaled( w / steps, s_stepBtnOn200->height(),
		                                    Qt::IgnoreAspectRatio,
		                                    Qt::SmoothTransformation );
		stepoff = s_stepBtnOff->scaled( w / steps, s_stepBtnOff->height(),
		                                Qt::IgnoreAspectRatio,
		                                Qt::SmoothTransformation );
		stepoffl = s_stepBtnOffLight->scaled(
		    w / steps, s_stepBtnOffLight->height(), Qt::IgnoreAspectRatio,
		    Qt::SmoothTransformation );
	...
	{
		p.drawLine( x_base + static_cast<int>( ppt * t ) - 1, TCO_BORDER_WIDTH,
		            x_base + static_cast<int>( ppt * t ) - 1,
		            TCO_BORDER_WIDTH + lineSize );
	...
	if( !( fixedTCOs() && beatPattern ) )
	{
		...
		// outer border
		p.setPen( ( current && !beatPattern ) ? c.lighter( 130 )
		                                      : c.darker( 300 ) );
